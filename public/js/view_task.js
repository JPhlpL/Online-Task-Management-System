...
$(document).ready(function () {
            ...
            },
            {
                    ...
                    return (
                        '<div class="btn-group-vertical" style="width:100%" role="group" aria-label="Task Actions">' +
                            ...
                            '<a href="' + viewTaskUrl('view_attach',row.task_attach_name) + '" id="view-button" name="view-button" class="btn btn-primary mt-1 view-button">View</a>' +
                        '</div>'
                    );
                },
            },
    ...
    // Assuming this is where you handle the "View" button click event
    $(document).on("click", ".view-button", function (e) {
        e.preventDefault(); // Prevent default link behavior

        var actionUrl = $(this).attr("href"); // Get the action URL from the button's href attribute

        // Send AJAX request to get the image URL
        $.ajax({
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            },
            ...
            success: function (response) {
                ...
                if (response.url) {
                    ...
                    // Handle error if the image URL is not available in the response
        ...
        Dropzone.options.myDropzone = {
            ...
            init: function() {
                this.on("success", function(file, response) {
                    // Reload the table after successful file upload
                    attachTable.ajax.reload();
